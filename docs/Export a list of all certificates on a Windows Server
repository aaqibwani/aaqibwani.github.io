---
title: Export a list of all certificates on a Windows Server to a CSV
nav_order: 13
---

# How to export a list of all certificates on a Windows Server

```
$array = @()
Get-ChildItem -Path Cert:\ -recurse | Where-Object {$_.PSISContainer -eq $false} | foreach-object ({
...
$array | Export-Csv -Path “c:\temp\temp.csv” -NoTypeInformation
```
